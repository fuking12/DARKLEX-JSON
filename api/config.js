export default function handler(req, res) {
  res.status(200).json({
    connectmg: "*🎬 Welcome to ᴅᴀʀᴋʟᴇx ᴍᴅ Movie Bot!* 🍿\n\n*🔥 Best Place for Movies & Entertainment*\nSearch and download your favorite movies easily.\n\n*✨ Available Features*\n• Latest Movies Updates\n• Fast Download Links\n• HD / Full HD Quality\n• Easy Movie Search\n\n*📢 Official Channel*\n👉 https://whatsapp.com/channel/0029Vb2ykk1GzzKRWrHROf1D\n\n*🛠️ Support Group*\n👉 https://chat.whatsapp.com/BVCFuD2qEXzGEatQzk41Ff\n\n━━━━━━━━━━━━━━━\n*> ᴅᴀʀᴋʟᴇx ᴍᴅ*",

    cmsglogo: "https://files.catbox.moe/j16kr3.jpg",
    footer: "*> ᴅᴀʀᴋʟᴇx ᴍᴅ*",
    chlink: "https://whatsapp.com/channel/0029Vb2ykk1GzzKRWrHROf1D",
    reponame: "",
    supglink: "https://chat.whatsapp.com/BVCFuD2qEXzGEatQzk41Ff",
    mainchanal: "120363380090478709@newsletter",
    megaurl: "https://mega.nz/file/yh5yhYYI#woOBsYxvdqHyMVvP488dbN5F3PUd3aBRs0G-BY29-ig",
    megaurl2: "https://mega.nz/file/URpjXJwQ#1X6KvOUQqiiKttI7k_ovO4SiicmEGRDFKqjcPvPM8oc",
    megaurlindex: "https://mega.nz/file/AQgECLiK#FqImK8xRQLcLSwXGWCdget_abL_DeJaFC3juthUsfLI",
    chreact: "❤️",
    connectmsg_sent: "94781209709@s.whatsapp.net",
    mvfree: "false"
  });
}
